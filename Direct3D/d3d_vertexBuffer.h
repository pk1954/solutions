...
public:
    ...
    m_fx(fx), m_fy(fy), m_fz(fz), m_dwColor(dwColor) 
{};
...
public:
    ...
    void  AddVertex( float const fx, float const fy, DWORD const dwColor) { m_vertexVector.push_back( Vertex( fx, fy, 0.0f, dwColor ) ); }
    void  ResetVertexBuffer( )                                            { m_vertexVector.clear(); }
    ULONG GetNrOfVertices( ) const                                        { return static_cast<ULONG>(m_vertexVector.size()); }
