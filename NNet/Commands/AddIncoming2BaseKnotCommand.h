...
class AddIncoming2BaseKnotCommand : public NNetCommand
{
public:
	AddIncoming2BaseKnotCommand
	(
		NobId         const   id, 
	...
	{ 
		...
		m_upPipe = MakePipe(m_upNew.get(), m_pOld);
		m_upNew->AddOutgoing(m_upPipe.get());
	}
	
	~AddIncoming2BaseKnotCommand() final = default;

	void Do() final 
	{ 
		m_pOld->AddIncoming(m_upPipe.get());
	...
	void Undo() final 
	{ 
		m_upNew  = m_pNMWI->PopFromModel<InputNeuron>();
...
private:
	...
	unique_ptr<InputNeuron> m_upNew  { };
