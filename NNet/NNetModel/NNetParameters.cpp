...
float Param::GetParameterValue(ParamType::Value const param) const
{
	switch (param)
	{
		...
		case stimulusMaxFreq: return m_stimulusMaxFreq.GetValue();
		case stimulusMaxTime: return m_stimulusMaxTime.GetValue();
		case baseFrequency:   return m_baseFrequency  .GetValue();
		...
		case peakVoltage:	  return m_peakVoltage    .GetValue();
...
)
{
	...
	{
		using enum ParamType::Value;
		case stimulusMaxFreq: m_stimulusMaxFreq = static_cast<fHertz     >(fNewValue); break;
	    case stimulusMaxTime: m_stimulusMaxTime = static_cast<fMicroSecs >(fNewValue); break;	    
		case baseFrequency:   m_baseFrequency   = static_cast<fHertz     >(fNewValue); break;
		...
		case peakVoltage:	  m_peakVoltage     = static_cast<mV         >(fNewValue); break;
