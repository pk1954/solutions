...
bool Param::operator==(Param const & rhs) const
{
	return
		...
		(m_stdPulseRate  == rhs.m_stdPulseRate ) &&
...
float const Param::GetParameterValue(ParamType::Value const param) const
{
	switch (param)
	{
		case ParamType::Value::stdPulseRate:   return m_stdPulseRate.GetValue();
...
)
{
	switch (param)
	{
		case ParamType::Value::stdPulseRate:   m_stdPulseRate  = static_cast<fHertz     >(fNewValue); break;
		...
		case ParamType::Value::signalLoss:     break;  // no longer used, only for backward compatibility
		default: assert(false);
	}
