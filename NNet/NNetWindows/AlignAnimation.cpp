...
bool AlignAnimation::AlignSelection( )
{
	...
	int iNrOfInputNeurons  { 0 };
	iNrOfInputNeurons = m_pNMRI->GetUPShapes().CountInSelection( ShapeType::Value::inputNeuron );
	...
	if ( (iNrOfInputNeurons == 0) && (iNrOfOutputNeurons == 0) )
		return false;
...

	{
		...
		umPntVectorTarget.Clear();
		umPntVectorActual.Clear();
		m_shapesAnimated.Clear();

		for ( int i = 0; i < points.size(); ++i )
		{
			umPntVectorTarget.Add( umPntTargetStart + umPntPackedSingle * static_cast<float>(i) );
			umPntVectorActual.Add( points[i].pBaseKnot->GetPosition() );
