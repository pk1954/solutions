...
{
    PixelPoint const pixDelta         = pixDesired - pixActual;
    PIXEL      const pixDistance      = MaxAbsDelta(pixDelta);
    PIXEL      const pixBreakDistCont = PIXEL((m_pixVelocity.GetValue() + 1) * m_pixVelocity.GetValue() / 2);   // break distance if we continue with actual velocity
    PIXEL      const pixBreakDistAcc  = pixBreakDistCont + m_pixVelocity + 1_PIXEL;               // break distance if we accelerate
    PIXEL      const pixDeltaV        = (pixDistance >= pixBreakDistAcc) 
    ...
    if (::abs(pixDelta.GetXvalue()) > ::abs(pixDelta.GetYvalue()))  // x is major direction
    {
        ...
        {
            auto pixVelocityMinor = PIXEL(abs((pixStepX.GetValue() * pixDelta.GetYvalue() + pixDelta.GetXvalue() / 2) / pixDelta.GetXvalue()));
    ...
    else  // y is major direction
    {
        ...
        {
            auto pixVelocityMinor = PIXEL(abs((pixStepY.GetValue() * pixDelta.GetXvalue() + pixDelta.GetYvalue() / 2) / pixDelta.GetYvalue()));
