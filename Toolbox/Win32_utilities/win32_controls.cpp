...
{
	HWND hwnd = CreateWindow(L"Static", text, WS_CHILD | WS_VISIBLE, iXpos, iYpos, iWidth, height, hwndParent, 0, GetModuleHandle(nullptr), 0);
	return hwnd;
}

HWND CreateEditField(HWND const hwndParent, int const iXpos, int const iYpos, int const iWidth, int const height)
{
	HWND hwnd = CreateWindow(L"Edit", 0, WS_CHILD|WS_VISIBLE|ES_RIGHT, iXpos, iYpos, iWidth, height, hwndParent, 0, GetModuleHandle(nullptr), 0);
	return hwnd;
}
