...
NobId UPNobList::Push(UPNob upNob)	
{
	...
	if (upNob)
	{
		...
		switch (upNob->GetNobType())
		{
			using enum NobType::Value;
			case inputConnector:  m_listInputConnector .push_back(*static_cast<InputConnector  const*>(upNob.get()));
			case outputConnector: m_listOutputConnector.push_back(*static_cast<OutputConnector const*>(upNob.get()));
			case inputLine:   	  m_listInputLine      .push_back(*static_cast<InputLine       const*>(upNob.get()));
			case outputLine:	  m_listOutputLine     .push_back(*static_cast<OutputLine      const*>(upNob.get()));
			case neuron:		  m_listNeuron         .push_back(*static_cast<Neuron          const*>(upNob.get()));
			case knot:		      m_listKnot           .push_back(*static_cast<Knot            const*>(upNob.get()));
			case fork:		      m_listFork           .push_back(*static_cast<Fork            const*>(upNob.get()));
			case synapse:		  m_listSynapse        .push_back(*static_cast<Synapse         const*>(upNob.get()));
			case pipe:		      m_listPipe           .push_back(*static_cast<Pipe            const*>(upNob.get()));
