...
public:
	...
	Nob * Front       ()               const { return   m_list[0]; }
	Nob * GetAt       (NobId const id) const { return   m_list[id.GetValue()]; }
//	Nob & GetRef      (NobId const id)       { return * m_list[id.GetValue()].get(); }
	...
	{
		for_each(m_list, [&f](Nob const* pNob) { if (pNob) f(*pNob); });
	}
	...
	) const // TODO: Template!
	{
		...
		idRes = findNobAt(umPoint, m_listInputConnector, crit);
		if (IsDefined(idRes))
			return idRes;

		idRes = findNobAt(umPoint, m_listOutputConnector, crit);
		if (IsDefined(idRes))
			return idRes;

		idRes = findNobAt(umPoint, m_listNeuron, crit);
		if (IsDefined(idRes))
			return idRes;

		idRes = findNobAt(umPoint, m_listInputLine, crit);
		if (IsDefined(idRes))
			return idRes;

		idRes = findNobAt(umPoint, m_listOutputLine, crit);
		if (IsDefined(idRes))
			return idRes;

		idRes = findNobAt(umPoint, m_listKnot, crit);
		if (IsDefined(idRes))
			return idRes;

		idRes = findNobAt(umPoint, m_listFork, crit);
		if (IsDefined(idRes))
			return idRes;

		idRes = findNobAt(umPoint, m_listSynapse, crit);
		if (IsDefined(idRes))
			return idRes;

		idRes = findNobAt(umPoint, m_listPipe, crit);
		if (IsDefined(idRes))
			return idRes;
...
private:
	...
	template <Nob_t T>
	NobId findNobAt(MicroMeterPnt const pnt, vector<T> const& list, auto const& crit) const
	{
		for (T const& n : list)
		{
			if (n.IsDefined() && n.Includes(pnt))
			{
				Nob const* pNob { &n };
	...
	vector<InputConnector > m_listInputConnector  {};
	vector<OutputConnector> m_listOutputConnector {};
	vector<InputLine      > m_listInputLine       {};
	vector<OutputLine     > m_listOutputLine      {};
	vector<Neuron         > m_listNeuron          {};
	vector<Knot           > m_listKnot            {};
	vector<Fork           > m_listFork            {};
	vector<Synapse        > m_listSynapse         {};
	vector<Pipe           > m_listPipe            {};
